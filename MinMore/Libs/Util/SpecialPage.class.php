...
class SpecialPage {
    ...
    function __construct($Total_Size = 1, $Page_Size = 20, $Current_Page = 1, $List_Page = 6, $PageParam = 'page', $pageRule = '', $static = FALSE, $static_size = 0) {
        //默认模板配置
        $this->Page_tpl['default'] = array('Tpl' => '<div class="Flip">当前{pageindex}/{pagecount}页{first}{prev}{liststart}{list}{listend}{next}{last}</div>', 'Config' => array());
        //每页显示信息数量
    ...
    protected function Pager($Page_tpl = '') {
        ...
        for ($i = $pStart; $i <= $pEnd; $i++) {
            ...
            //此处不分静态动态页面
            if ($i == $cfg ['pageindex']) {
                $pList .= '<a class="kjk_x" href="#">' . str_replace('*', $i, $cfg['list']) . '</a>';
            } else {
                //此处是为了照顾静态地址生成时，第一页不显示当前分页1，启用该方法，静态地址需要$this->pageRule传入的是array，并且包含两个 index,list。index是首页规则,list是其他分页规则
                if ($this->Static && $i == 1) {
                    $pList .= '<a class="kjk" href="' . $this->pageRule['index'] . '">' . str_replace('*', $i, $cfg['list']) . '</a>';
                } else {
                    $pList .= '<a class="kjk" href="' . str_replace('*', $i, $cfg['link']) . '">' . str_replace('*', $i, $cfg['list']) . '</a>';
                }
            }
        }
        ...
        if ($cfg['listsidelong'] > 0) {
            if ($cfg ['listsidelong'] < $pStart) {
                for ($i = 1; $i <= $cfg ['listsidelong']; $i++) {
                    if ($this->Static && $i == 1) {
                        $pListStart .= '<a class="kjk" href="' . $this->pageRule['index'] . '">' . str_replace('*', $i, $cfg['list']) . '</a>';
                    } else {
                        $pListStart .= '<a class="kjk" href="' . str_replace('*', $i, $cfg['link']) . '">' . str_replace('*', $i, $cfg ['list']) . '</a>';
                    }
                }
            ...
            if (($cfg['pagecount'] - $cfg['listsidelong']) > $pEnd) {
                ...
                for ($i = (($cfg ['pagecount'] - $cfg['listsidelong']) + 1); $i <= $cfg['pagecount']; $i++) {
                    if ($this->Static && $i == 1) {
                        $pListEnd .= '<a class="kjk" href="' . $this->pageRule['index'] . '">' . str_replace('*', $i, $cfg['list']) . '</a>';
                    } else {
                        $pListEnd .= '<a class="kjk" href="' . str_replace('*', $i, $cfg ['link']) . '">' . str_replace('*', $i, $cfg['list']) . ' </a>';
                    }
                }
            } else {
        ...
        if ($cfg['pageindex'] > 1) {
            //第一页链接的HTML代码
            if ($this->Static) {
                $pFirst = '<a class="shajji" href="' . $this->pageRule['index'] . '">' . $cfg['first'] . '</a>';
            } else {
                $pFirst = '<a class="shajji" href="' . str_replace('*', 1, $cfg['link']) . '">' . $cfg['first'] . '</a>';
            }
            //显示上一页HTML代码
            //如果生成静态，且上一页为首页时
            if ($this->Static && ($cfg['pageindex'] - 1) == 1) {
                $pPrev = '<a class="shajji" href="' . $this->pageRule['index'] . '">' . $cfg ['prev'] . '</a> '; //显示首页
            } else {
                //显示上一页
                if ($this->Static_Size && $cfg['pageindex'] - 1 <= $this->Static_Size) {
                    $pPrev = '<a class="shajji" href="' . str_replace('*', $cfg['pageindex'] - 1, $this->urlParameters($_GET)) . '">' . $cfg['prev'] . '</a>';
                } else {
                    $pPrev = '<a class="shajji" href="' . str_replace('*', $cfg['pageindex'] - 1, $cfg['link']) . '">' . $cfg['prev'] . '</a>';
                }
            }
        }
        ...
        if ($cfg ['pageindex'] < $cfg ['pagecount']) {
            //最后一页
            $pLast = '<a class="shajji" href="' . str_replace('*', $cfg['pagecount'], $cfg['link']) . '">' . $cfg['last'] . '</a>';
            //下一页
            //如果下一页还是在生成静态页访问内
            if ($this->Static_Size && $cfg['pageindex'] + 1 <= $this->Static_Size) {
                $pNext = '<a class="shajji" href="' . str_replace('*', $cfg['pageindex'] + 1, $this->urlParameters($_GET)) . '">' . $cfg['next'] . '</a>';
            } else {
                $pNext = '<a class="shajji" href="' . str_replace('*', $cfg['pageindex'] + 1, $cfg['link']) . '">' . $cfg['next'] . '</a>';
            }
        }
