...
class ContentController extends AdminBase {
    ...
    public function index() {
        ...
        $typeRs = $SpecialType->where(array('parentid' => $this->specialid, 'role'=>$role))->select();
    ...
    public function edit() {
        if (IS_POST) {
            ...
            $SpecialType = D('SpecialType');
            $role = get_site_role();
            $typeRs = $SpecialType->where(array('parentid' => $info['specialid'], 'role'=>$role))->select();
            $typeRs = $typeRs ? $typeRs : array();
            $types = array();
            foreach ($typeRs as $r) {
                $types[$r['typeid']] = $r['name'];
    ...
    public function import() {
        if (IS_POST) {
            ...
            //取得模型数据
            $models = D('Model')->field('modelid,name')->select();
