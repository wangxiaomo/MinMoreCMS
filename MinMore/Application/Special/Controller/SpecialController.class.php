...
class SpecialController extends AdminBase {
    ...
    protected function _initialize() {
        ...
        //取得当前内容模型模板存放目录
        $this->filepath = TEMPLATE_PATH . (empty(\Common\Controller\MinMoreCMS::$Cache["Config"]['theme']) ? "Default" : \Common\Controller\MinMoreCMS::$Cache["Config"]['theme']) . DIRECTORY_SEPARATOR . "Special" . DIRECTORY_SEPARATOR;
        //专题首页模板
    ...
    public function index() {
        ...
        //信息总数
        $count = $this->special->where($where)->count();
        $page = $this->page($count, 20);
        $data = $this->special->where($where)->limit($page->firstRow . ',' . $page->listRows)->order(array("listorder" => "ASC", "id" => "DESC"))->select();
    ...
    public function add() {
        if (IS_POST) {
            ...
            //站点ID
            $special['siteid'] = get_site_role();
            //分类
            $special['type'] = I('post.type', '', '');
            //添加用户信息
            $user = User::getInstance()->getInfo();
            $special['username'] = $user['username'];
            $special['userid'] = $user['id'];
            $special['role'] = get_site_role();
    ...
    public function edit() {
        if (IS_POST) {
            ...
            //取得专题分类数据
            $typeData = D('SpecialType')->where(array('role' => get_site_role(), 'parentid' => $specialid))->select();
            if (empty($typeData)) {
                $typeData = array();
            }
            $type = array();
    ...
    public function html() {
        ...
        if ($act) {
            $spid = I('get.spid', 0);
            $ids = F('_special_html_' . $uid);
            if ($act == 'index') {
                if ($ids[$spid]) {
                    $specialid = $ids[$spid];
                    //生成首页
                    $SpecialHtml->index($specialid);
                ...
                if ($specialid) {
                    ...
                    $typeid = $SpecialType->where(array('parentid' => $specialid,'role'=>get_site_role()))->getField('typeid', true);
                    if($typeid){
                        foreach($typeid as $tid){
                            $SpecialHtml->type($tid);
                        }
                    }
