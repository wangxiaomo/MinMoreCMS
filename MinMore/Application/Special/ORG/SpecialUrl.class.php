...
class SpecialUrl {
    ...
    private $siteId, $siteConfig;

    function __construct() {
        //设置当前站点
        $this->siteId = get_site_role();
        $this->siteConfig = get_site_config();
        if (false === $this->siteConfig) {
            D("Config")->config_cache($this->siteId);
            $this->siteConfig = F("ConfigSite_" . $this->siteId);
        }
    }
    ...
    public function index($data, $page = 1) {
        //站点域名
        $domain = $this->siteConfig['siteurl'];
        //站点存放目录
        $dirname = $this->siteConfig['dirname'];
        //取分页最大值
        $page = max($page, 1);
        //生成专题URL规则
        $urlrule = array();
        if ($data['ishtml']) {
            $site_path = $dirname? $dirname . '/': '';
            $urlrule[0] = $site_path . 'special/' . $data['filename'] . '/index.html';
            $urlrule[1] = $site_path . 'special/' . $data['filename'] . '/index_{$page}.html';
        } else {
    ...
    public function type($data, $special, $page = 1) {
        ...
        if ($special['ishtml']) {
            $site_path = $dirname? $dirname . '/': '';
            $urlrule[0] = $site_path . 'special/' . $special['filename'] . '/' . $data['typedir'] . '/index.html';
            $urlrule[1] = $site_path . 'special/' . $special['filename'] . '/' . $data['typedir'] . '/index_{$page}.html';
        } else {
    ...
    public function show($data, $special, $page = 1) {
        ...
        if ($special['ishtml']) {
            //取得URL规则
            $site_path = $dirname? $dirname . '/': '';
            $urlrule = $site_path . 'special/{$year}/{$month}/{$id}.html|special/{$year}/{$month}/{$id}_{$page}.html';
        } else {
            //取得URL规则
