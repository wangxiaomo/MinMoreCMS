...
<title>添加新文章 - 系统后台 - {$Config.sitename} - by MinMoreCMS</title>
...
<div class="wrap J_check_wrap">
  ...
  <div class="col-right">
    <div class="table_full">
      <table width="100%">
         <tr>
          <td><div  style="text-align: center;"><input type='hidden' name='info[thumb]' id='thumb' value='{$info.thumb}'>
			...
			<img src='<if condition=" $info['thumb'] ">{$info.thumb}<else/>{$config_siteurl}statics/images/icon/upload-pic.png</if>' id='thumb_preview' width='135' height='113' style='cursor:hand' /></a>
                       <br/>  <input type="button" class="btn" onclick="crop_cut_thumb($('#thumb').val());return false;" value="裁减图片"> 
            <input type="button"  class="btn" onclick="$('#thumb_preview').attr('src','{$config_siteurl}statics/images/icon/upload-pic.png');$('#thumb').val('');return false;" value="取消图片"><script type="text/javascript">
	...
	if ( id =='' || id == undefined ) { 
        ...
        <tr>
          <td><?php echo Form::date('info[updatetime]',date('Y-m-d H:i:s',$info['updatetime']),1); ?></td>
        </tr>
      </table>
    </div>
  </div>
    ...
    <div class="table_full">
      <table width="100%">
            <tr>
              ...
              <td><span class="must_red">*</span><?php echo Form::select($types,$info['typeid'],"name='info[typeid]'",'请选择所属分类'); ?></td>
            </tr>
            <tr>
              <th width="80">
                标题
               </th>
              <td><span class="must_red">*</span><input type="text" style="width:400px;" name="info[title]" id="title" value="{$info.title}" style="color:" class="input input_hd J_title_color" placeholder="请输入标题"/></td>
            </tr>
            <tr>
              <th width="80">
                关键词 
            ...
            <tr>
              ...
              <td><input type="text" name="linkurl" id="linkurl" value="<if condition=" $info['islink'] ">{$info.url}</if>" style="width:280px;"maxlength="255" disabled class="input length_3"> <input name="info[islink]" type="checkbox" id="islink" value="1" <if condition=" $info['islink'] ">checked</if> onclick="ruselinkurl();" > <font color="red">转向链接</font></td>
            </tr>
            <tr>
               <th width="80"> 
               摘要
               </th>
               <td><textarea name="info[description]" id="description" style='width:98%;height:46px;' >{$info.description}</textarea> </td>
             </tr>
             <tr>
              <th width="80">
                内容 
               </th>
              <td><span class="must_red">*</span><script type="text/plain" id="content" name="info[content]">{$info.content}</script><?php echo Form::editor('content','full','Special'); ?></td>
            </tr>
        </tbody>
      </table>
    </div>
  ...
  <div class="btn_wrap" style="z-index:999;text-align: center;">
    <div class="btn_wrap_pd">
      ...
      <input type="hidden" name="info[specialid]" value="{$info.specialid}" />
									...
									button:[
										{
											name: '继续添加？',
											callback:function(){
												location.href = "{:U('Content/add',array('specialid'=>$specialid))}";
												return true;
											},
											focus: true
										},{
