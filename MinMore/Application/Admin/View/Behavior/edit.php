...
<div class="wrap">
  ...
  <div class="prompt_text">
    ...
    <ul>
      ...
      <li>module->行为所属模块，没有该参数时，自动定位到 MinMore\Common\Behavior 目录。</li>
    </ul>
    <p><b>规则定义格式3：</b> </p>
    <ul style="color:#00F">
    ...
    <p><b>规则字段解释：</b></p>
    <ul>
      <li>sql-><literal>需要执行的SQL语句，表前缀可以使用“minmore_”代替。参数可以使用 {$parameter}的形式（该形式只对行为标签参数是为数组的有效，纯碎的参数使用{$self}）！</literal></li>
      <li>cycle->执行周期，单位（小时），表示$cycle小时内最多执行$max次 </li>
      <li>max->单个周期内的最大执行次数（$cycle和$max必须同时定义，否则无效）</li>
    </ul>
  </div>
