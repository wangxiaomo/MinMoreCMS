...
class IndexController extends Base {
    ...
    public function _initialize() {
        ...
        $this->assign('Title', C('MINMORE_APPNAME'))
                ->assign('Powered', 'Powered by www.minmore.com');
    }
    ...
    public function step_2() {
        ...
        $folder = array(
            '/',
            '/d/',
            '/MinMore/Application/Install/',
            '/MinMore/Common/Conf/',
            '/MinMore/Common/Conf/addition.php',
        );
        $folderInfo = array();
    ...
    public function mysql() {
        ...
        //读取数据文件
        $sqldata = file_get_contents(MODULE_PATH . 'Data/minmore.sql');
        //读取测试数据
        if ($testdata) {
            $sqldataDemo = file_get_contents(MODULE_PATH . 'Data/minmore_demo.sql');
            $sqldata = $sqldata . "\r\n" . $sqldataDemo;
        } else {
            //不加测试数据的时候，删除d目录的文件
            try {
        ...
        $query = "INSERT INTO `{$dbPrefix}user` VALUES ('1', '{$username}', '未知', '{$password}', '', '{$time}', '0.0.0.0', '{$verify}', 'admin@www.minmore.com', '备注信息', '{$time}', '{$time}', '1', '1', '');";
        mysql_query($query);
