...
class ShareModel extends Model {
    ...
    public function shareAdd($userid, $catid, $data) {
        ...
        $id = MinMoreCMS()->Content->add($data);
        if ($id) {
            //前台投稿，根据栏目配置和用户配置
            ...
            $this->error = MinMoreCMS()->Content->getError();
            return false;
        }
    }
    ...
    public function shareEdit($userid, $catid, $id, $data) {
        ...
        $id = MinMoreCMS()->Content->edit($data);
        if ($id) {
            return $id;
        } else {
            $this->error = MinMoreCMS()->Content->getError();
    ...
    public function shareDel($userid, $id) {
        ...
        define("GROUP_MODULE", "Contents");
        if (MinMoreCMS()->Content->delete($info['content_id'], $info['catid'])) {
            $this->where(array('userid' => $userid, 'id' => $id))->delete();
            return true;
        } else {
            $this->error = MinMoreCMS()->Content->getError() ? : '信息删除失败！';
            return false;
        }
    }
