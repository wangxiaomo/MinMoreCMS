...
<div class="wrap J_check_wrap">
  ...
  <form name="myform" action="{:U("reply","isadmin=1")}" method="post" class="J_ajaxForm">
    ...
    <table width="100%" class="table_form contentWrap">
        ...
        <tr>
          <th>联系方式</th>
          <td>Email：{$info.yxdz}， 手机：{$info.sjhm}</td>
        </tr>
        <tr>
          <th>信件内容</th>
          <td>{$info.xxnr}</td>
	...
	<volist name="comments" id="vo">
        ...
        <tr>
          <th>回复时间</th>
          <td>
		<if condition="$info.hfnr NEQ ''">
		{$info.hfsj|date="Y-m-d H:i:s",###}
		<else/>
		<p>暂无</p>
		</if>
	</td>
        ...
        <tr>
          <th>回复内容</th>
          <td><textarea name="reply" style="width:600px; height:200px;" id="reply">{$info.hfnr}</textarea></td>
        </tr>
      </tbody></table>
    </div>
	...
	<a class="btn btn_submit mr10" href="javascript:void(0);" onclick="showSubDept('{$info.id}','{$info.xjzt}')" value="{$info.id}">转发>></a>
      </div>
    </div>
  </form>
</div>
...
<div class="title" style="text-align:center;font-size:20px;color:white;float:left;width:80%">
  ...
  <div class="close_modal">  
	...
	<input type="text" name="type" value="{$info.type}" style="display:none"></input>
	</div>
	<div class="fd_item">
