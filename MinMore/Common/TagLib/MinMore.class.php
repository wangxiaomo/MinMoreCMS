...
class MinMore extends TagLib {

    // 数据库where表达式
    protected $comparisonMinMorecms = array(
        '{eq}' => '=',
        '{neq}' => '<>',
        '{elt}' => '<=',
        '{egt}' => '>=',
     ...
     * 标签：<get></get>
     * 作用：特殊标签，SQL查询标签
     * 用法示例：<get sql="SELECT * FROM minmore_article  WHERE status=99 ORDER BY inputtime DESC" page="$page" num="5"> .. HTML ..</get>
     * 参数说明：
    ...
    public function _get($tag, $content) {
        ...
        $tag['sql'] = $sql = str_replace(array("think_", "minmore_"), C("DB_PREFIX"), strtolower($tag['sql']));
        //数据源
...
//                    $parseStr .= ' $get_db->db(1,"' . $db . '"); ';
                ...
                if ($_sql = preg_replace('/select([^from].*)from/i', "SELECT COUNT(*) as count FROM ", $tag['sql'])) {
                    ...
                    if (substr(trim($sql), 0, 1) == '$') {
                        $parseStr .= ' $sql = str_replace(array("think_", "minmore_"), C("DB_PREFIX"),' . $sql . ');';
...
//                        $parseStr .= ' $get_db->db(1,"' . $db . '"); ';
            ...
            if ($table) {
                ...
                //判断是否变量传递
                if (substr(trim($sql), 0, 1) == '$') {
                    $parseStr .= ' $_sql = str_replace(array("think_", "minmore_"), C("DB_PREFIX"),' . $sql . ');';
                } else {
                    $parseStr .= ' $_sql = "' . str_replace('"', '\"', $sql) . '";';
                }
                $parseStr .= ' $get_db = M();';
    ...
    protected function parseSqlCondition($condition) {
        $condition = str_ireplace(array_keys($this->comparisonMinMorecms), array_values($this->comparisonMinMorecms), $condition);
        return $condition;
    }
